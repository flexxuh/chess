...
public class Piece {
    ...
    public boolean isMoveLegal(Board board, int endRow, int endCol) {
        switch (this.character) {
            ...
            case '\u2659':
            case '\u265f':
                //checks if pawn promotion is needed by seeing if in row 0 or 7
                if(endRow == 0 || endRow == 7){
                    if(board.getPiece(endRow,endCol) == null){
                        Scanner scan = new Scanner(System.in);
                        System.out.println("What piece would you like to change to");
                        String nPiece = scan.nextLine().toLowerCase();
                        if(nPiece.startsWith("r")){
                            Rook rook = new Rook(endRow,endCol, isBlack);
                        }else if(nPiece.startsWith("k")){
                            Knight knight = new Knight(endRow,endCol,isBlack);
                        }
                        else if(nPiece.startsWith("b")){
                            Bishop bishop = new Bishop(endRow,endCol,isBlack);
                        }else if(nPiece.startsWith("q")){
                            Queen queen = new Queen(endRow,endCol,isBlack);
                        }
                    }
                }
